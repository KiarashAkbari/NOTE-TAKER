/* ==========================================================================
   ...
   Uses Google Identity Services (GIS) token client — client-side only,
   no backend, no client secret. Free, no billing account required.
   ========================================================================== */
...
async function onTokenResponse(resp) {
  if (resp.error) {
    setSyncState(accessToken ? 'synced' : 'offline');
    return;
  }
...
function scheduleTokenRefresh() {
  ...
  // Refresh silently a bit before the ~1hr token expiry
  syncTimer = setTimeout(() => requestToken(false), 50 * 60 * 1000);
}
